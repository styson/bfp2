...
function Lightbox({ src, src2, title, onClose }: { src: string; src2?: string; title: string; onClose: () => void }) {
  ...
  return (
    <div className='fixed inset-0 z-[100] bg-black/85 flex items-start justify-center p-10 pt-12 overflow-y-auto' onClick={onClose}>
      ...
      {part.image && (
        ...
        >
          <img
            src={part.image}
            alt={part.name}
            className='w-full h-20 object-contain px-2 pt-2 transition-opacity duration-200 group-hover:opacity-70'
          />
          {part.image2 && (
            <img
              src={part.image2}
              alt={`${part.name} B`}
              className='w-full h-20 object-contain px-2 pb-2 transition-opacity duration-200 group-hover:opacity-70'
            />
          )}
      ...
      {part.image && (
        ...
        >
          <img
            src={part.image}
            alt={part.name}
            className='w-full h-32 object-contain p-2 transition-opacity duration-200 group-hover:opacity-70'
          />
          <div className='absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200'>
            <ZoomIn className='text-[#f0b429]' />
          </div>
...
export const PartsSection = () => {
  ...
  const shipping = parts.filter((p) => p.type === 'shipping');
